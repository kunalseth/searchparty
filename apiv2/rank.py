...
class SearchParty(DataImport):
    ...
    def company_search(self, df, value):
        ...
        """
        index = df['Company Name'].str.contains(value, case=False, na=False)
        match_company = list(df[index].head(1)['Company Name'].astype(str))[0]
    ...
    def sort(self, df, scores):
        ...
        sorted_companies = df.sort_values(by="scores", ascending=False).reset_index()
        return sorted_companies.fillna('').head(50)
    ...
    def similar_comp(self, filtered_data, company_name, print_time=True):
        """
        ...
        index = self.df[self.df['Company Name'] == company_name].index
        filtered_indices = filtered_data.index.values

        # compute cosine similarity
        similarity_scores = cosine_similarity(self.tfidf[index], self.tfidf[filtered_indices])
        
        # Reshape array for the shape of the df
        similarity_scores = np.reshape(similarity_scores, len(filtered_data))
